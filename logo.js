...
// textpro("3D-sci-fi", "caca").then((data) => {
//     console.log(data)
// })
...
export async function textpro(effect, ...texts) {
//   texts = texts.filter(v => v)
//   let eff = effects.find(v => (new RegExp(v.title, 'gi')).test(effect))
//   if (!eff) return -1
  let resCookie = await fetch("https://textpro.me/3d-chrome-text-effect-827.html", {
    headers: {
  ...
  let resUrl = await fetch("https://textpro.me/3d-chrome-text-effect-827.html", {
    method: "POST",
    headers: {
      Accept: "*/*",
